...
def build() -> dict:
    ...
    wl = metrics.winners_losers(br["_returns"], constituents, n=10)
    sec_movers = metrics.sector_movers(br["_returns"], constituents, n=5)
    fm = metrics.fx_and_macro(prices, week_end)
    fi = metrics.fixed_income(prices, week_end)
    ...
    report = {
        "meta": {
            ...
            "y10_close": next((c["close"] for c in fi["curve"] if c["label"] == "10 años"), None),
            "y10_change_bps": next((c["change_bps"] for c in fi["curve"] if c["label"] == "10 años"), None),
