...
# Curva de rendimientos de treasuries (tickers de Yahoo, en %). Se reporta el
# cierre y la variación en puntos básicos en la semana.
TREASURY_CURVE = {
    "^IRX": "3 meses",
    "^FVX": "5 años",
    "^TNX": "10 años",
...
# Proxies de crédito vía ETFs (retorno semanal, no OAS/spread real — no
# tenemos esa fuente sin FRED). LQD = investment grade, HYG = high yield,
# EMB = soberano/corporativo emergente.
CREDIT_ETFS = {
...
def all_price_tickers(constituents: pd.DataFrame) -> list[str]:
    ...
    tickers += list(MACRO)
    tickers += list(FX)
    tickers += list(TREASURY_CURVE)
    tickers += list(CREDIT_ETFS)
